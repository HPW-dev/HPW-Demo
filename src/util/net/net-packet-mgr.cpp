...
struct Packet_mgr::Impl {
  ...
  inline void connect_to(cr<Target_info> target) {
    ...
    if (ep == _last_binded_addr) {
      hpw_info << "TCP connection with " << target.ip_v4 << ":" << target.port << " already established";
      return;
    }

    hpw_info << "TCP connect to " + target.ip_v4 + ":" << target.port;
  ...
  inline void _init_udp(cr<Config> cfg) {
    ...
    hpw_info << "UDP connection setup to " + _status.ip_v4 + ":" << _status.port_udp;
  }
  ...
  inline void _init_tcp(cr<Config> cfg) {   
    ...
    hpw_info << "TCP connection setup to " + _status.ip_v4 + ":" << _status.port_tcp;
  }
