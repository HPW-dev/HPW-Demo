...
void Menu::update(double dt) {
  iferror(m_items.empty(), "Menu.update: m_items is empty");
...
void Menu::prev_item() {
  iferror(m_items.empty(), "Menu.prev_item: m_items is empty");
  if (m_cur_item == 0) {
...
void Menu::next_item() {
  iferror(m_items.empty(), "Menu.next_item: m_items is empty");
  ++m_cur_item;
