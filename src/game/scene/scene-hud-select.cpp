...
struct Scene_hud_select::Impl {
  ...
  inline explicit Impl() {
    ...
    cauto TITLE = get_locale_str("hud_select.title");
  ...
  inline void draw_bg(Image& dst) const {
    ...
    cauto bg = hpw::sprites.find("resources/image/HUD/" + graphic::cur_hud + ".png");
    if (bg) {
    ...
    dst.fill(Pal8::black);
    assert(graphic::font);
    graphic::font->draw(dst, get_screen_center() - Vec(30, 0), get_locale_str("hud_select.no_image"));
  }
}; // Impl
