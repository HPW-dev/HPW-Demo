...
int load_bar_sz(double val, double max, int size_bar) {
  return std::ceil(safe_div<double, double>(val, max) * size_bar);
}

/// рисует прозрачный текст с чёрными контурами
void draw_expanded_text(Image& dst, CN<utf32> txt, const Vec pos) {
  // рендер в буфферы под текст
  Image hp_overlay(graphic::font->text_width(txt) + 2, graphic::font->text_height(txt) + 2,
    Pal8::black);
