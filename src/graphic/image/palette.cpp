...
inline static Rgbr pal_average(cr<Palette> pal24, size_t start, size_t end) {
  iferror (pal24.size() < 256, "< 256 colors in palette file (" +
    std::to_string(pal24.size()) + ")");
  iferror (start >= end, "color start >= end");
  iferror (start >= pal24.size(), "color start >= pal.size");
  iferror (end >= pal24.size(), "color end >= pal.size");
    
  Rgbr avr;
  for (size_t i = start; i < end; ++i)
    avr += rgb24_to_real(pal24[i]);
  cauto sz = end - start;
  avr /= sz;
  return avr;
}

inline static real normalize_hue(real hue) {
  if (hue < 0)
    hue = 360.0 - hue;
...
bool by_similar(cr<Palette> a, cr<Palette> b) {
  // берём две половинки основной части палитры и ищем их средние значения
  cauto a_avg_l = pal_average(a, 0, Pal8::gray_size/2);
  cauto a_avg_r = pal_average(a, Pal8::gray_size/2, Pal8::gray_size);
  cauto b_avg_l = pal_average(b, 0, Pal8::gray_size/2);
  ...
  if (int(a_hue) * 6 == int(b_hue) * 6)
    return to_luma(a_avg) < to_luma(b_avg);
