...
Pal8 rgb_to_pal8(int R, int G, int B) {
  ...
  for (size_t i = 0; i < pal8_default_table.size(); ++i) {
    ...
    const int dr = pal.r - R;
    const int dg = pal.g - G;
    const int db = pal.b - B;
...
Rgb24 to_rgb24(const Pal8 x) {
  return pal8_default_table.at(static_cast<size_t>(x));
} // to_rgb24
