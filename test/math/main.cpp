...
int main() {
  try {
    ...
    real a = 0.97997;
    real b = 3.000095;
    log_info << ("a: " << n2s(a, 8) << ", " << std::hex << to_u32(a) << std::endl);
    log_info << ("b: " << n2s(b, 8) << ", " << std::hex << to_u32(b) << std::endl);

    auto mul_ab = a * b;
    log_info << ("a * b: " << n2s(mul_ab, 8) << ", " << std::hex << to_u32(mul_ab) << std::endl);

    auto pow2_a = pow2(a);
    auto pow2_b = pow2(b);
    log_info << ("pow2 a: " << n2s(pow2_a, 8) << ", " << std::hex << to_u32(pow2_a) << std::endl);
    log_info << ("pow2 b: " << n2s(pow2_b, 8) << ", " << std::hex << to_u32(pow2_b) << std::endl);

    auto pow_ab = std::pow(a, b);
    log_info << ("pow(a, b): " << n2s(pow_ab, 8) << ", " << std::hex << to_u32(pow_ab) << std::endl);

    auto sqrt_a = std::sqrt(a);
    auto sqrt_b = std::sqrt(b);
    log_info << ("sqrt a: " << n2s(sqrt_a, 8) << ", " << std::hex << to_u32(sqrt_a) << std::endl);
    log_info << ("sqrt b: " << n2s(sqrt_b, 8) << ", " << std::hex << to_u32(sqrt_b) << std::endl);

    const real pi = PI;
    log_info << ("pi: " << n2s(pi, 8) << ", " << std::hex << to_u32(pi) << std::endl);

    real sum_x3 = a;
    sum_x3 += a;
    sum_x3 += a;
    log_info << ("sum a x3: " << n2s(sum_x3, 8) << ", " << std::hex << to_u32(sum_x3) << std::endl);

    real mul_x3 = a;
    mul_x3 *= a;
    mul_x3 *= a;
    log_info << ("mul a x3: " << n2s(mul_x3, 8) << ", " << std::hex << to_u32(mul_x3) << std::endl);
    ...
    log_info << ("sin a: " << n2s(sin_a, 8) << ", " << std::hex << to_u32(sin_a) << std::endl);
    log_info << ("sin b: " << n2s(sin_b, 8) << ", " << std::hex << to_u32(sin_b) << std::endl);
    log_info << ("cos a: " << n2s(cos_a, 8) << ", " << std::hex << to_u32(cos_a) << std::endl);
    log_info << ("cos a: " << n2s(cos_b, 8) << ", " << std::hex << to_u32(cos_b) << std::endl);
    log_info << ("atan2(a, b): " << n2s(atan2_ab, 8) << ", " << std::hex << to_u32(atan2_ab) << std::endl);
    
    real c = 0.12345678;
    real d = 54321.0123;
    log_info << ("c: " << n2s(c, 8) << ", " << std::hex << to_u32(c) << std::endl);
    log_info << ("d: " << n2s(d, 8) << ", " << std::hex << to_u32(d) << std::endl);
    ...
    log_info << ("c + d: " << n2s(sum_cd, 8) << ", " << std::hex << to_u32(sum_cd) << std::endl);
    log_info << ("c - d: " << n2s(sub_cd, 8) << ", " << std::hex << to_u32(sub_cd) << std::endl);
    log_info << ("c * d: " << n2s(mul_cd, 8) << ", " << std::hex << to_u32(mul_cd) << std::endl);
    log_info << ("c / d: " << n2s(div_cd, 8) << ", " << std::hex << to_u32(div_cd) << std::endl);
    log_info << ("sin(d): " << n2s(sin_d, 8) << ", " << std::hex << to_u32(sin_d) << std::endl);
    log_info << ("sqrt_d(d): " << n2s(sqrt_d, 8) << ", " << std::hex << to_u32(sqrt_d) << std::endl);

    auto pos_1 = simple_physics();
    auto pos_2 = simple_physics_2();
    log_info << ("pos 1: " << pos_1.x << " (" << std::hex << to_u32(pos_1.x)
      << "), " << pos_1.y << " (" << std::hex << to_u32(pos_1.y) << ")\n");
    log_info << ("pos 2: " << pos_2.x << " (" << std::hex << to_u32(pos_2.x)
